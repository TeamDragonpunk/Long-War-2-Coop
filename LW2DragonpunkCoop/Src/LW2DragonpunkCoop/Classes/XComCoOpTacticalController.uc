...
{	
	...
	else
	{
		...
		{
			`log("Testing Unit:" @TempUnit.GetFullName(),,'Dragonpunk Coop LW2');
			`log("HasServerConnection:" @`XCOMNETMANAGER.HasServerConnection() @"Flight Fuel:"@(TempUnit.GetMaxStat(eStat_FlightFuel)),,'Dragonpunk Coop LW2');
			`log("HasClientConnection:" @`XCOMNETMANAGER.HasClientConnection() @"Flight Fuel:"@(TempUnit.GetMaxStat(eStat_FlightFuel)),,'Dragonpunk Coop LW2');

			if(`XCOMNETMANAGER.HasServerConnection() && (TempUnit.GetMaxStat(eStat_FlightFuel)==10))
				EligibleUnits.RemoveItem(TempUnit);
			else if (`XCOMNETMANAGER.HasClientConnection() && (TempUnit.GetMaxStat(eStat_FlightFuel)!=10))
				EligibleUnits.RemoveItem(TempUnit);
		}
	}
	...
	`log("EligibleUnits Length:" @EligibleUnits.Length @", bAllowedToSwitch:"@bAllowedToSwitch,,'Dragonpunk Coop LW2');
	...
	while (EligibleUnits.Length > 0 && bAllowedToSwitch)
	{
		...
		`log("Next SelectNewUnit is:" @SelectNewUnit.GetFullName() ,,'Dragonpunk Coop LW2');
...
simulated function bool Visualizer_SelectPreviousUnit()
{
	...
	while( EligibleUnits.Length > 0 && CurrentUnit.ReflexActionState != eReflexActionState_SelectAction )
	{
		...
		SelectNewUnit = EligibleUnits[CurrentSelectedIndex];
		`log("Previous SelectNewUnit is:" @SelectNewUnit.GetFullName() ,,'Dragonpunk Coop LW2');
		if (Visualizer_SelectUnit(SelectNewUnit))
			return true;
...
simulated function bool Visualizer_SelectUnit(XComGameState_Unit SelectedUnit)
{
	...
	{
		`log("IsCurrentlyWaiting",,'Dragonpunk Coop LW2');
	...
	{
		`log("AllowVisualizerSelection False",,'Dragonpunk Coop LW2');
		return false;
	}
	`log(SelectedUnit.GetFullName() @"Passed all tests!" ,,'Dragonpunk Coop LW2');
...
defaultproperties
{
	InputClass=class'LW2DragonpunkCoop.XComCoOpInput'
}